    ...
    <div className={`${darkMode?"pageDark":""}`}>
      <div className={` center flex flex-col items-center my-10 px-4`}>
        ...
        <div className="w-full max-w-6xl  justify-center   h-[356px] rounded sm:hidden mobile:hidden 2xl:flex xl:flex lg:flex md:flex  flex-col gap-6 p-6 shadow-md">
          {/* Table Header */}
          <div className={`${darkMode?"darkTheme text-white":"text-black"} hidden z-10  md:grid grid-cols-4 gap-4 bg-slate-100 p-4 rounded text-center md:text-left font-semibold `}>
            <div>Product</div>
            <div>Price</div>
            <div>Quantity</div>
            <div>Subtotal</div>
          ...
          <div className="space-y-4 md:space-y-0  h-auto overflow-y-auto flex flex-col gap-6 ">
            {/* First Product Row */}
            <div className={`${darkMode?"darkTheme text-white":""} flex  mobile:items-center flex-col md:grid grid-cols-4 gap-4 bg-slate-100 p-4 rounded`}>
              <div className={`${darkMode?"":""} flex sm:justify-center mobile:justify-center items-center md:gap-2 gap-4`}>
                <img className="w-16 h-12" src={led} alt="LCD Monitor" />
                <p className={`${darkMode?"text-white":""} text-gray-700 md:text-xs md:font-normal font-medium`}>
                  LCD Monitor
                </p>
              </div>
              <div className={`${darkMode?"text-white":""} text-gray-700 text-center md:text-left flex items-center`}>
                $650
              </div>
            ...
            {/* Second Product Row */}
            <div className={`${darkMode?" darkTheme":""} flex flex-col mobile:items-center md:grid grid-cols-4 gap-4 bg-slate-100 p-4 rounded`}>
        ...
        <div className="w-full xl:flex-row md:flex-row  2xl:flex-row lg:flex-row max-w-6xl flex sm:flex-col mobile:flex-col   justify-between mt-">
          <button className={`${darkMode?"bg-black text-white":"bg-white text-black"} px-6  2xl:w-[218px] 2xl:h-[56px] xl:w-[218px] xl:h-[56px] lg:w-[218px] lg:h-[56px] md:w-[218px] md:h-[56px]  py-3 text-base sm:my-2 mobile:my-2  active:bg-red-700 active:text-white  font-medium border border-black rounded  `}>
            Return To Shop
          </button>
          <button className={`${darkMode?"bg-black text-white":"bg-white text-black"} px-6  2xl:w-[218px] 2xl:h-[56px] xl:w-[218px] xl:h-[56px] lg:w-[218px] lg:h-[56px] md:w-[218px] md:h-[56px]  py-3 text-base sm:my-2 mobile:my-2  active:bg-red-700 active:text-white  font-medium border border-black rounded `}>
            Update Cart
        ...
        <div className="w-full  max-w-6xl flex sm:flex-col lg:flex-row mobile:flex-col  mt-10 gap-6">
          ...
          {/* Total Section */}
          <div className={`${darkMode?"bg-black text-white":" bg-white"} lg:w-1/3 border border-gray-400 p-6 rounded shadow-md `}>
            <h2 className="font-bold text-xl mb-4">Cart Total</h2>
            <div className={`${darkMode?"bg-black text-white":"bg-white text-black"} space-y-4 text-gray-700`}>
