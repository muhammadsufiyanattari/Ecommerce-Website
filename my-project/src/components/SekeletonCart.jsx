...
export function SekeletonCart({
  ...
  {/* <Link id={id} to={`/productDetail/${id}`}> */}
    ...
    >
     ...
     <div  className=" sm:w-[172px] w-[120px] h-auto    sm:h-[155px] m-auto  ">
     ...
     rounded- bg-slate-400 animate-pulse           h-[40px] text-white text-xs sm:text-lg w-[100%] flex   items-end justify-center p-2 rounded-b`}
      ...
      <div className={` myp  sm:w-auto sm:h-auto w-[150px]  h-auto  gap-0 `}>
        <h2
          className={`
  rounded bg-slate-300 animate-pulse
           w-auto h-[30px] sm:text-sm font-medium transition-all duration-300   overflow-hidden text-[10px] bg-yellow   `}
        >
          
        </h2>
        <p className="flex gap-4 my-2 rounded bg-slate-300 animate-pulse w-[100px] h-6 sm:text-base text-[12px]  font-medium text-myTheme">
         
        </p>
        <div className="star-img flex gap-2 w-20 h-5 bg-slate-300 rounded animate-pulse ">
              {/* <img src={star} alt="" /> */}
              {/* <span>{starRank}</span> */}
            </div>
